Attribute VB_Name = "SourceControlHelper"
Option Explicit
...
Private Function IsExportable(ByRef Cmp As VBIDE.VBComponent) As Boolean
    ...
    If ArrayHelper.Exists(Cmp.Type, ExportableComponentsTypes) = False Then
        IsExportable = False
        Exit Function
    End If
...
' The reason why number of lines has to be bigger than 2 is that
' we want to count as an empty file even these files which have
' Explicit Option enabled and one empty line after option.
'
' Files which have following structure will be counted as non-empty.
' Option Explicit
...
Private Function IsComponentEmpty(ByRef Source As VBIDE.VBComponent) As Boolean
    ...
    If Source.CodeModule.CountOfLines <= 2 Then
        If VBA.LCase$(Source.CodeModule.Lines(1, 1)) = "option explicit" _
                                                  And Source.CodeModule.Lines(2, 1) = vbNullString Then
...
Private Function IsImportable(ByRef File As Scripting.File) As Boolean
    ...
    If ArrayHelper.Exists(CmpType, ImportableComponentsTypes) = False Then
        IsImportable = False
        Exit Function
    End If
